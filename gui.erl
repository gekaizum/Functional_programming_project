-module(gui).
-export([start/0]).
-export([handle_click/2, start_sim/4]).
...
start() ->
	...
	Label_1 = wxStaticText:new(Frame_1, ?wxID_ANY, "Number of Cells"),
...
handle_click(#wx{obj = Button, userData = #{input_1 := Input_1,input_2 := Input_2, env := Env}}, _Event) ->
	...
	case Label of
		...
		"Start" ->
			...
			spawn(?MODULE, start_sim, [Input_1, Input_2, Button, Env]);
...
start_sim(Input_1, Input_2, Button, Env) ->
	
	wx:set_env(Env),
	N = list_to_integer(wxTextCtrl:getValue(Input_1)),
	Frame_size = list_to_integer(wxTextCtrl:getValue(Input_2)),
	% Create a new ETS named 'main_ets'
	ets:new(main_ets,[public,named_table]),
	
	% Insert random cells into the 'main_ets' table	
    	insert_cells(N,Frame_size),		

	% Create a new frame (window)
	Frame = wxFrame:new(wx:null(), 1, "Life_and_Evolution_of_cells",[{size,{Frame_size,Frame_size}}]),
	...
	wxFrame:show(Frame),
	canvas_loop(Frame_size, Button, main_ets, Frame, BmpGeneral, BmpCeed, BmpLeaf, BmpAntena, BmpRoot, N, 50).
	

%% Function to continuously update and display the canvas
canvas_loop(_Frame_size, _Button,_main_ets,Frame, _BmpGeneral, _BmpCeed, _BmpLeaf , _BmpAntena , _BmpRoot,_N,0) ->
	ets:delete(main_ets),				
	wxFrame:destroy(Frame),	
	ok;	
canvas_loop(Frame_size, Button,main_ets, Frame, BmpGeneral, BmpCeed, BmpLeaf, BmpAntena, BmpRoot, N, I) ->
	case wxButton:getLabel(Button) of
		"Stop" ->
			...
			canvas_loop(Frame_size, Button, main_ets, Frame, BmpGeneral, BmpCeed, BmpLeaf, BmpAntena, BmpRoot, N, I-1);
		"Start" ->
...
insert_cells(N,Frame_size) ->
	...
	ets:insert(main_ets, {{rand:uniform(Frame_size),rand:uniform(Frame_size)}, general}),
	ets:insert(main_ets, {{rand:uniform(Frame_size),rand:uniform(Frame_size)}, ceed}),
	ets:insert(main_ets, {{rand:uniform(Frame_size),rand:uniform(Frame_size)}, leaf}),
	ets:insert(main_ets, {{rand:uniform(Frame_size),rand:uniform(Frame_size)}, antena}),
	ets:insert(main_ets, {{rand:uniform(Frame_size),rand:uniform(Frame_size)}, root}),

	% Recursively insert (N-1) points
	insert_cells(N - 1, Frame_size).
